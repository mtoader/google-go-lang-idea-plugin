...
package com.goide.inspections.external.govet;
...
public class GoVetExternalAnnotator extends ExternalAnnotator<GoVetExternalAnnotator.State, GoVetExternalAnnotator.State> {
  private final static Logger LOG = Logger.getInstance(GoVetExternalAnnotator.class);
  ...
  public State collectInformation(@NotNull PsiFile file) {
    ...
    HighlightDisplayKey key = HighlightDisplayKey.find(GoVetInspection.INSPECTION_SHORT_NAME);
  ...
  public void apply(@NotNull PsiFile file, @Nullable State annotationResult, @NotNull AnnotationHolder holder) {
    ...
    for (Problem problem : annotationResult.problems) {
      ...
      String message = "go vet: " + problem.myDescription;
      ...
      annotation.registerFix(new DisableInspectionToolAction(key) {
        ...
        public String getName() {
          return "Disable 'go vet-based inspections'";
        }
      });
    }
  }
  ...
  private ProcessOutput getExecutorOutput(@NotNull State state) throws ExecutionException {
    return GoExecutor.in(state.myModule)
      .withParameters("vet", state.myFilePath)
      .withTimeout(2000)
