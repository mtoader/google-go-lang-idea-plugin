...
public class GoModuleBuilder extends JavaModuleBuilder implements SourcePathsBuilder, ModuleBuilderListener {
    ...
    public void moduleCreated(@NotNull final Module module) {
        ...
        GoToolWindow toolWindow = GoToolWindow.getInstance(module.getProject());
        toolWindow.setTitle(TITLE);
        toolWindow.showAndCreate(module.getProject());

        String packageDir;
        if (isNew) {
            packageDir = createNewPackage(module, toolWindow, sourceRoots, projectName);
        } else {
            packageDir = downloadPackage(module, toolWindow, sourceRoots, projectName);
        }

        if (packageDir.isEmpty()) {
            LOG.error("Could not properly create the project");
            return;
        }

        createProjectSettings(module, toolWindow, packageDir);
    }

    private String createNewPackage(final Module module, final GoToolWindow toolWindow, final VirtualFile[] sourceRoots, final String projectName) {
        ...
        String packageDir = mainPackage.getVirtualFile().getPath();
    ...
    private String downloadPackage(final Module module, final GoToolWindow toolWindow, final VirtualFile[] sourceRoots, final String projectName) {
        ...
        if (projectDir == null) {
            LOG.error("Could not retrieve the project directory");
            return "";
        }

        String packageDir = sourceRoots[0].getCanonicalPath() + File.separatorChar + packageURL;
    ...
    private void createProjectSettings(Module module, GoToolWindow toolWindow, String packageDir) {
        String configName = (new File(packageDir)).getName();
